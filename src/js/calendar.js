        ...
        $(calendar)
            .fullCalendar({
                ...
                viewRender: function (view, element) {
                    var monthAndYear = $(calendar).fullCalendar('getDate').format('MMMM YYYY');
                    // Render date / month
                    $(el).find('.calendar-monthYear').text(monthAndYear);
                    ...
                    $(calendar).find('.fc-day-number').each(function () {
                        ...
                        $(day).addClass(eventClass).html('<div class="calendar-daySquare"><div class="calendar-event"><span class="fa ' + iconClass + '"/></div><div class="calendar-dayNumber">' + text + '</div></div>');
                    });
                },
                loading: function (isLoading, view) {
                    if (isLoading) {
                        $(loader).fadeIn(80);
                        $(calendar).find('.calendar-event').css('opacity', 0);
                    } else {
                        $(loader).fadeOut(80);
                        $(calendar).find('.calendar-event').css('opacity', '');
                    }
                },
            ...
            .on('calendar:goto', function (e, params) {
                var gotoInput = document.activeElement.classList.contains('calendar-goto') ? document.activeElement : null;
                var date;
                // Update the calendar.
                if (params.duration) {
                ...
                if (!params.selection) {
                    if (params.action === 'prev' || params.action === 'next' || (params.duration && params.duration.months)) {
                ...
                calendar.querySelector('.fc-day-number[data-date="' + date.format('YYYY-MM-DD') + '"]').classList.add('calendar-day--active');
                // Display availability message in goto input.
                if (gotoInput) {
                    var dateEl = calendar.querySelector('.fc-day-number[data-date="' + date.format('YYYY-MM-DD') + '"]');
                    ...
                    if (dateEl.classList.contains('calendar-dayUnavailable')) {
                        ...
                        if (dateEl.classList.contains('fc-past')) {
                            calendarMsgEl.innerHTML = '<span class="fa fa-exclamation-circle"></span> <strong>Umm... ' + date.format('dddd, MMM Do') + '? </strong> Please try a date in the future. <span class="fa fa-smile-o"></span>';
                        } else if (dateEl.classList.contains('fc-sun')) {
                            calendarMsgEl.innerHTML = '<span class="fa fa-exclamation-circle"></span> <strong>Sorry, we are closed on Sundays.</strong> Please try another date.';
                        } else {
                            calendarMsgEl.innerHTML = '<span class="fa fa-exclamation-circle"></span> <strong>Sorry, ' + date.format('dddd, MMM Do') + ' is unavailable.</strong> Contact us to be notified if it becomes available.';
                        }
                        ...
                        calendarMsgEl.innerHTML = '<span class="fa fa-check-circle"></span> <strong>' + date.format('dddd, MMM Do') + ' is available!</strong> Send this form to reserve it.';
                    }
        ...
        $(gotoDateChoosers)
            ...
            .on('keyup', function (e) {
                ...
                // Escape key closes calendar-callout.
                    ...
                    gotoInput.value = $(calendar).fullCalendar('getDate').format('MM/DD/YYYY');
                }
